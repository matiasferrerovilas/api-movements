...
- `monthly_summary_seen`: per (workspace, user, year, month) marker that a user dismissed that
  month's close screen — backs `GET /v1/users/me` → `metadata.pendingMonthlySummary`. The monthly
  summary itself is computed on-demand with `SUM` against `movements`, not cached.
