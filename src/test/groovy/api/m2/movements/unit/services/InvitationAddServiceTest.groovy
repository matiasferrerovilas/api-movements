...
class InvitationAddServiceTest extends Specification {
    ...
    InvitationAddService service

    def setup() {
        service = new InvitationAddService(
                workspaceAddService,
    ...
    def "inviteToWorkspace - should throw PermissionDeniedException when caller is not member of workspace"() {
        given:
        ...
        when:
        service.inviteToWorkspace(1L, ["invited@test.com"])
    ...
    def "inviteToWorkspace - should create invitations when caller is member of workspace"() {
        given:
        def caller = User.builder().id(10L).email("caller@test.com").build()
        def workspace = Workspace.builder().id(1L).name("Mi cuenta").owner(caller).build()
